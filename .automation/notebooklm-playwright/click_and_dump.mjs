...
try {
  ...
  const controls = await page.locator('button, a, [role="button"], input, textarea').evaluateAll((els) =>
    els
      .map((el) => {
        ...
        return [type, aria, placeholder, text].filter(Boolean).join(' | ');
      })
      .filter(Boolean)
      .slice(0, 200)
  );

  const safe = clickText ? sanitizeFileStem(clickText) : 'state';
  const screenshotPath = path.join(outDir, `notebooklm-${safe}.png`);
