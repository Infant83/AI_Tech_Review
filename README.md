...
- Worker 구현은 `.automation/cloudflare/ai-tech-review-public-metrics/src/worker.js`에 있다. D1의 `page_counts`, `daily_counts` 테이블에 경로별 `views`, `active_seconds`, `engagement_events`, `max_scroll_percent`만 저장하고, `/summary`가 허브와 카드의 조회수 및 평균 읽은 시간을 반환한다.
- API는 `https://infant83.github.io`와 로컬 개발 origin만 CORS로 허용하고, 집계 경로도 `/`, `/ko.html`, `/AI_Tech_Review/...`로 제한한다. IP, User-Agent, 쿠키, 개인 식별자는 저장하지 않는다.
