...
![분자 후보와 ML 예측, QUBO 기반 양자 어닐링 배치 선택, 양자화학 검증을 분리해 묘사한 과학 일러스트](molecular_inverse_design_hero.png)
...
![QM9 데이터부터 ML, 분자 생성, QUBO와 QPU 배치 선택, PySCF DFT, 고정 라벨 replay 및 제안된 live loop를 연결한 상세 인포그래픽](molecular_inverse_design_infographic.svg)

*그림 2. 위쪽과 가운데는 실행된 계산을, 아래 점선은 다음 검증 단계로 제안한 live active-learning loop를 나타냅니다. 수치는 [공개 계산 결과표](benchmark_results.csv)와 동일합니다.*

[상세 인포그래픽을 전체 크기 PNG로 보기](molecular_inverse_design_infographic.png)
...
![같은 QM9 test split에서 Chemprop D-MPNN과 ExtraTrees Morgan 모델의 예측값과 참조 orbital gap을 비교한 parity plot](02_qm9_surrogate_parity.png)
...
![세 분자 생성 경로의 validity, uniqueness, novelty, 공통 화학 필터 통과율과 기록된 proposal 시간을 비교한 차트](03_generator_metrics.png)
...
![같은 frozen acquisition BQM에서 exact, classical simulated annealing, D-Wave QPU의 solution gap과 기록된 wall time을 비교한 차트](04_acquisition_comparison.png)
...
![exact-selected batch와 D-Wave QPU-selected batch의 분자별 DFT target error, 평균 오차, 최선 오차를 비교한 차트](07_exact_vs_qpu_dft_batch.png)
...
![QPU batch 세 분자의 Chemprop 예측과 PySCF orbital gap을 구조식과 함께 비교한 차트](05_dft_molecule_validation.png)
...
![40개에서 52개까지 공개한 QM9 label 수에 따라 random, greedy, exact, QUBO simulated annealing의 best target error가 변하는 convergence plot](06_qm9_replay_convergence.png)
