...
과학 결과는 논문의 초록을 쉬운 말로 바꾸는 것만으로 충분하지 않습니다. 독자가 먼저 알아야 할 것은 이 연구가 어떤 실제 문제 때문에 시작됐고, 이전에는 무엇이 어려웠으며, 이번 결과로 무엇을 새로 판단할 수 있게 되었는가입니다. `중요하다`, `의미가 크다`고 선언하는 대신 달라진 계산, 측정 또는 의사결정을 구체적으로 씁니다.
...
이 순서를 `배경–방법–결과–의미–한계`라는 고정 목차로 복사하지는 않습니다. 독자가 자연스럽게 질문을 따라가도록 문단 안에 배치합니다. LinkedIn 게시물과 보도자료는 주제를 발견하고 설명 방식을 참고하는 자료이며, 과학적 주장과 수치는 원 논문·Supplementary Information·공식 코드와 데이터에서 다시 확인합니다.

`검증 경로`, `증거 지도`, `연구 축`, `평가 레이어`처럼 작업자의 분류를 나타내는 말은 독자용 문장에서 기본값으로 쓰지 않습니다. 예를 들어 “하나의 검증 경로”라고 묶기보다 “광격자 실험과 고전 계산이 같은 확산계수를 냈는지 비교했다”고 씁니다. `검증`이라는 기술어가 실제 통계 절차나 실험–계산 대조를 뜻할 때는 그대로 사용합니다.

글의 성격에 따라 다음 모듈에서 필요한 것만 배치합니다.
...
- 2026.08: 글별 에이전트·하네스 공개, TeX source 안전 게이트, interactive·video 기준, 공개 asset allowlist와 제목·수식·표·metadata 게시 검사를 추가했습니다. 8월 31일에는 과학 결과를 실제 문제–기존 병목–연구진이 한 일–정량 결과–의미–한계의 순서로 설명하고, `검증 경로` 같은 추상적인 작업자 용어를 구체적인 대상과 동사로 바꾸는 규칙을 보완했습니다.
