...
<figure class="article-hero-figure">
  <img class="diagram-desktop" src="candidate_journey_quantum_specialists.svg" alt="구조와 조건, 예측값, 불확실성, 탈락 이유를 담은 재료 후보 작업지시서가 생성, 빠른 예측, 고정밀 계산, 선택, 실험 공정을 순환하며 각 병목에서 선택적 양자 전문 모듈을 만나는 과정">
  <img class="diagram-mobile" src="candidate_journey_quantum_specialists_mobile.svg" alt="모바일 화면용 세로 배열로 재구성한 재료 후보 작업지시서의 폐루프 여정과 선택적 양자 전문 모듈">
  <figcaption><strong>그림 1.</strong> 후보 하나는 분자식만 적힌 카드가 아니라 구조·조건·예측·불확실성·탈락 이유를 함께 싣고 공정을 도는 작업지시서에 가깝습니다. 양자컴퓨터는 공장 전체를 바꾸는 장비가 아니라, 전자구조·학습·표본추출·이산 선택의 특정 병목에 연결하는 전문 공정입니다.</figcaption>
</figure>

재료 역설계의 출발 질문은 간단합니다. **원하는 물성을 내는 구조와 공정 조건을 어떻게 거꾸로 찾아낼 것인가?** 실제 답을 찾는 과정은 목표와 제약을 정하고, 후보를 만들고, 빠르게 거른 뒤, DFT·양자화학·분자동역학 계산과 실험으로 확인하는 폐루프입니다. 탈락한 후보도 버려지지 않습니다. 왜 실패했는지가 다음 후보를 만드는 정보가 됩니다.

이 흐름을 하나의 작업지시서가 여러 전문 공정을 오가는 과정으로 생각하면 양자컴퓨팅의 위치가 선명해집니다. QPU가 전체 공정을 한 번에 대신하는 것은 아닙니다. 강한 상관 전자상태를 풀거나, 작은 자료에서 새로운 유사도 표현을 시험하거나, 복잡한 후보 분포를 표본화하거나, 제약이 많은 조합을 고르는 지점에 선택적으로 들어갑니다. 따라서 핵심 질문도 “양자컴퓨터를 쓸 것인가”가 아니라 **어느 계산을, 어떤 입력과 출력으로, 전체 비용까지 포함해 더 낫게 만들 수 있는가**입니다.

<div class="reader-layers" aria-label="이 리뷰를 읽는 세 단계">
  <div class="reader-layer"><strong>1 · 기본 지도</strong><span>기존 재료정보학 폐루프의 어디에서 계산비용과 불확실성이 커지는지 먼저 봅니다.</span></div>
  <div class="reader-layer"><strong>2 · 작동 원리</strong><span>VQE·QPE·QML·표본추출·QA·QAOA가 무엇을 입력받아 어떤 방식으로 답을 만드는지 연결합니다.</span></div>
  <div class="reader-layer"><strong>3 · 채택 판단</strong><span>표현력이 아니라 같은 예산에서 검증을 통과한 후보가 늘었는지로 양자 이득을 판정합니다.</span></div>
</div>

## 현재의 재료 역설계는 강한 고전 폐루프에서 출발합니다

실제 프로젝트의 순서는 분야마다 조금씩 다르지만, 먼저 고정해야 할 조건은 비슷합니다. 문제정의 단계에서 목표 물성, 합성·안전·원가·공정 제약, 입력 자료, 기대 출력, 허용오차와 검증법을 정합니다. 자료와 표현 단계는 구조, 조성, 계산조건과 측정값의 계보를 맞춥니다. 후보 생성과 빠른 예측이 탐색 범위를 줄이고, DFT·TDDFT·post-HF·MD 같은 고정밀 계산이 전자상태와 물성을 다시 판정합니다. 다목적 선별과 실험 설계는 제한된 계산·합성 예산을 어디에 쓸지 정하며, 실제 측정이 마지막 기준이 됩니다.
...
<summary>한눈에 보는 네 개입점 · 용어가 낯설면 상세 절을 읽은 뒤 펼쳐도 됩니다</summary>
...
**양자 파이프라인**은 각 양자 계산에 맞는 입력을 만들고 QPU 작업·측정·오류제어·후처리·재료 검증을 연결하는 실행 계층입니다. 결과가 기준을 넘지 못하면 기존 고전 해법으로 복귀하는 경로도 여기에 포함됩니다. 하나의 QPU가 네 계산을 모두 맡는 구조를 뜻하지 않습니다.
...
1. **기준선과 계산 경계를 고정합니다.** 자료 준비부터 재검증까지의 시간·비용을 계측하고, geometry·integral·active space, 재료 split, energy model 또는 QUBO 가운데 하나의 입력·출력과 오차 예산을 고정합니다.
2. **한 모듈을 기존 해법과 나란히 실행합니다.** 실제 의사결정에는 검증된 고전 결과를 사용하면서, 작은 active-space VQE, 고정 energy model의 annealer 표본추출 또는 제한된 QUBO의 QA·QAOA 결과를 함께 기록합니다. 이후 문제 크기·상관 난도·제약밀도를 늘려 강한 고전 해법과 전체 비용을 비교합니다.
...
후보 작업지시서가 고정밀 검사 공정에 도착했다고 생각해 보겠습니다. DFT·HF와 orbital 분석은 전체 구조와 주변 환경을 살펴 강한 상관·결함·전하이동이 예상되는 후보 영역을 제안합니다. 연구자는 orbital localization, 자연점유수, entanglement 지표와 화학적 판단을 함께 보고 active space를 고정합니다. QPU는 이렇게 정한 작은 영역의 상관 전자상태를 맡는 전문 장비입니다. 검사 결과인 에너지와 필요한 관측량은 다시 고전 계산과 다음 후보 선택으로 돌아갑니다.

:::

DFT와 양자컴퓨팅 사이의 전환은 `DFT 코드를 QFT로 바꾸는 일`이 아닙니다. DFT는 전자밀도와 교환–상관 범함수로 바닥상태 문제를 근사하는 전자구조 방법입니다. QFT(Quantum Fourier Transform)는 양자 레지스터의 basis를 바꾸는 회로 연산이며, [표준 QPE에서 누적된 고유위상을 이진수로 읽는 데](https://quantum.cloud.ibm.com/learning/modules/computer-science/qft) 쓰입니다. 분자와 재료의 에너지를 계산하는 알고리즘은 VQE·QPE이고, QFT는 그중 QPE 내부의 판독 서브루틴입니다.

계산화학자가 먼저 정해야 할 것은 알고리즘보다 **계산 경계**입니다. 구조 최적화, Kohn–Sham SCF, 주기 경계조건, pseudopotential, orbital localization과 embedding 환경은 고전 코드에 남깁니다. 그 결과에서 강한 상관, 결함 준위, 전하이동 또는 여기상태에 중요한 orbital만 골라 finite-basis active-space Hamiltonian을 만듭니다. QPU는 보통 DFT 전체가 아니라 CASSCF·CASCI·DMRG·selected CI가 맡던 상관 전자 solver 자리에 들어갑니다.
...
고정핵 Born–Oppenheimer 근사와 유한 spin-orbital basis에서 전자 Hamiltonian은 다음과 같이 씁니다. <span class="math-inline"><var>h</var><sub>pq</sub></span>에는 운동에너지와 전자–핵 인력이, <span class="math-inline"><var>g</var><sub>pqrs</sub></span>에는 전자–전자 반발 적분이 들어갑니다. 핵–핵 반발 에너지 <span class="math-inline"><var>E</var><sub>nuc</sub></span>는 상수항입니다.

::: evidence QPU에 전달되는 전자구조 문제

<div class="math-display" role="math" aria-label="전자 해밀토니안 H e는 핵 반발 에너지와 일전자 항, 이전자 항의 합입니다">
  <div class="math-row"><var>Ĥ</var><sub>e</sub> = <var>E</var><sub>nuc</sub> Î + <span class="sum">∑</span><sub>p,q</sub> <var>h</var><sub>pq</sub> â<sup>†</sup><sub>p</sub> â<sub>q</sub></div>
  <div class="math-row math-indent">+ <span class="frac"><span>1</span><span>2</span></span> <span class="sum">∑</span><sub>p,q,r,s</sub> <var>g</var><sub>pqrs</sub> â<sup>†</sup><sub>p</sub> â<sup>†</sup><sub>q</sub> â<sub>s</sub> â<sub>r</sub></div>
</div>

양자 알고리즘은 연속 공간의 Schrödinger 방정식 전체가 아니라, 선택한 basis·orbital·active space로 정의된 이 행렬의 고유상태 문제를 풉니다.

:::

Active space에 `K`개의 spatial orbital이 있으면 보통 <span class="math-inline"><var>n</var>=2<var>K</var></span>개의 spin orbital이 생깁니다. 전자수가 <span class="math-inline"><var>N</var><sub>e</sub></span>일 때 전자수만 고정한 determinant 공간은 <span class="math-inline">C(<var>n</var>, <var>N</var><sub>e</sub>)</span>로 커집니다. 예를 들어 CAS(20e,20o)는 이 기준에서 약 `1.38 × 10¹¹`개 determinant이고, 직접 점유수 encoding에는 symmetry tapering 전 40개의 qubit이 필요합니다. 이것은 지수적으로 큰 coefficient 공간을 담는 표현 용량입니다. 아직 계산 이득은 아닙니다. 좋은 상태를 준비하고, Hamiltonian을 적용하며, 필요한 관측량을 읽는 비용이 함께 작아야 합니다.
...
<figure class="figure-panel">
  <img class="diagram-desktop" src="electronic_structure_quantum_bridge.svg" alt="SCF와 주기계 환경에서 active-space 적분을 만들고, 2차양자화 Hamiltonian과 fermion-to-qubit 매핑을 거쳐 VQE 또는 QPE로 에너지와 RDM을 구한 뒤 고전 outer loop로 돌려보내는 계산 흐름">
  <img class="diagram-mobile" src="electronic_structure_quantum_bridge_mobile.svg" alt="모바일 화면용 세로 배열로 재구성한 DFT와 양자 active-space solver 사이의 계산 흐름">
  <figcaption><strong>그림 3.</strong> 고전 SCF와 embedding이 상관 active space를 정의해 QPU에 전달합니다. 양자 solver는 에너지와 필요한 RDM·관측량을 돌려주고, geometry·orbital·embedding은 고전 outer loop에서 갱신됩니다.</figcaption>
</figure>

### 2. Fermion 문제를 qubit 연산으로 번역합니다

전자 점유수는 0과 1이므로 qubit에 바로 적을 수 있어 보입니다. 그러나 전자는 서로 자리를 바꿀 때 부호가 달라지는 fermion입니다. 단순한 점유 장부만 옮겨 적으면 이 규칙을 잃습니다. Fermion-to-qubit mapping은 **어느 orbital이 찼는지와 앞선 orbital들의 parity를 함께 기록하는 새 장부 양식**입니다.

Jordan–Wigner mapping은 앞선 orbital의 parity를 `Z` string에 기록합니다. Bravyi–Kitaev mapping은 점유수와 parity 정보를 tree 구조로 나누어 spin-orbital 수 `n`에 대한 한 fermion 연산자의 최악 locality를 `O(n)`에서 `O(log n)`로 낮춥니다. 다만 실제 gate 수는 orbital ordering, symmetry tapering, hardware connectivity와 compiler에 달려 있으므로 작은 문제에서 어느 mapping이 항상 짧다고 말할 수는 없습니다. [Seeley·Richard·Love의 원 연구](https://doi.org/10.1063/1.4768229)는 이 변환을 전자구조 Hamiltonian에 구체적으로 적용했습니다.

Mapping 뒤에는 서로 교환하지 않는 Pauli string의 합이 남습니다.

::: evidence Qubit Hamiltonian

<div class="math-display" role="math" aria-label="큐비트 해밀토니안 H q는 파울리 문자열 P j의 가중합입니다">
  <div class="math-row"><var>Ĥ</var><sub>q</sub> = <span class="sum">∑</span><sub>j</sub> <var>c</var><sub>j</sub> <var>P̂</var><sub>j</sub><span class="condition"><var>P̂</var><sub>j</sub> ∈ {Î, X̂, Ŷ, Ẑ}<sup>⊗n</sup></span></div>
</div>

여기서 <span class="math-inline"><var>P̂</var><sub>j</sub></span>는 서로 다른 qubit의 <span class="math-inline">Î, X̂, Ŷ, Ẑ</span>가 곱으로 이어진 Pauli string입니다. 일반적인 전자구조 Hamiltonian에는 `X`와 `Y`가 들어갑니다. 뒤에서 다루는 QUBO 비용 Hamiltonian은 <span class="math-inline">Î, Ẑ<sub>i</sub>, Ẑ<sub>i</sub>Ẑ<sub>j</sub></span> 항만 있는 대각 문제이므로 계산 대상이 다릅니다.
...
VQE는 악기를 조율하는 과정과 닮았습니다. Ansatz는 악기의 구조이고, 회로 매개변수 `θ`는 조율 손잡이입니다. QPU가 현재 설정에서 여러 Pauli 항을 반복 측정해 에너지를 추정하면, 고전 optimizer가 손잡이를 다시 조정합니다. 이 순환은 낮은 에너지를 찾아가지만, 악기 구조인 ansatz가 목표 상태를 표현하지 못하거나 optimizer가 잘못된 골짜기에 머물면 바닥상태를 보장하지 않습니다.

계산은 Hartree–Fock determinant 또는 다중구성 reference <span class="math-inline">|<var>φ</var><sub>ref</sub>⟩</span>에서 시작합니다. Ansatz, 즉 탐색할 trial wavefunction의 형태를 매개변수 회로 `U(θ)`로 정하고, Pauli 항의 expectation value를 측정한 뒤 고전 optimizer가 `θ`를 갱신합니다.

::: evidence VQE의 변분 목적함수

<div class="math-display" role="math" aria-label="VQE 상태와 변분 에너지 목적함수">
  <div class="math-row">|<var>ψ</var>(<var>θ</var>)⟩ = Û(<var>θ</var>) |<var>φ</var><sub>ref</sub>⟩</div>
  <div class="math-row"><var>E</var>(<var>θ</var>) = ⟨<var>ψ</var>(<var>θ</var>)| Ĥ<sub>q</sub> |<var>ψ</var>(<var>θ</var>)⟩</div>
  <div class="math-row math-indent">= <span class="sum">∑</span><sub>j</sub> <var>c</var><sub>j</sub> ⟨<var>P̂</var><sub>j</sub>⟩<sub>θ</sub> ≥ <var>E</var><sub>0</sub></div>
</div>

마지막 부등식은 noise가 없고 expectation value를 정확히 구한 이상적인 경우의 Rayleigh–Ritz 상한입니다. Finite-shot 추정값과 오류완화 결과가 매 iteration에서 이를 보장하는 것은 아닙니다.

:::

[VQE 최초 실험](https://www.nature.com/articles/ncomms5213)의 중요한 전환은 긴 coherent time evolution을 비교적 짧은 상태 준비 회로와 반복 측정으로 바꾼 데 있습니다. UCC 계열 ansatz는 전자 excitation 구조를 보존하지만 orbital 수와 excitation rank에 따라 gate 수가 빠르게 늘어납니다. Hardware-efficient ansatz는 회로를 줄일 수 있지만 전자수·spin·size consistency와 화학적 해석성을 잃을 수 있습니다. ADAPT-VQE는 필요한 operator를 순차적으로 고르는 대신 후보 gradient를 측정하는 비용을 더 냅니다.

Energy 측정도 한 번의 readout이 아닙니다. 동일한 회로를 반복 실행하는 횟수를 shots라고 부릅니다. Pauli 항을 독립 표본화하는 기본 구조에서는 목표 표준오차 `ε`에 필요한 shots가 대체로 `1/ε²`에 비례합니다. [Huggins 등의 측정 연구](https://www.nature.com/articles/s41534-020-00341-7)는 low-rank factorization과 basis rotation grouping으로 조사한 계에서 측정 횟수를 크게 낮췄지만, 회로 반복·분산·정밀도의 거래관계는 남았습니다. Geometry마다 수십·수백 번 optimizer가 에너지를 다시 요청한다면 단일-point 데모의 비용을 그대로 해석할 수 없습니다.
...
QPE는 에너지 고유상태를 찾아 헤매는 optimizer가 아니라 **위상에 새겨진 주파수를 읽는 디지털 분석기**에 가깝습니다. 여러 주파수가 섞인 신호에서 특정 성분을 읽듯, 초기상태에 포함된 각 고유상태는 시간에 따라 <span class="math-inline"><var>E</var><sub>k</sub>/ℏ</span>의 각주파수로 위상을 누적합니다. QPE는 이 위상을 이진수로 판독합니다. 실제 분광 실험을 모사한다는 뜻은 아니며, 목표 고유상태와 충분히 겹치는 초기상태가 먼저 필요합니다.

::: evidence QPE의 위상 판독

<div class="math-display" role="math" aria-label="QPE 초기상태 분해와 해밀토니안 시간진화의 위상">
  <div class="math-row">|<var>φ</var>⟩ = <span class="sum">∑</span><sub>k</sub> <var>α</var><sub>k</sub> |<var>E</var><sub>k</sub>⟩</div>
  <div class="math-row">Û(<var>t</var>)|<var>E</var><sub>k</sub>⟩ = exp(−<var>iE</var><sub>k</sub><var>t</var>/ℏ)|<var>E</var><sub>k</sub>⟩</div>
  <div class="math-row"><var>w</var><sub>k</sub> = |<var>α</var><sub>k</sub>|<sup>2</sup></div>
</div>

<span class="math-inline">|<var>α</var><sub>k</sub>|<sup>2</sup></span>는 초기상태가 고유상태 `k`를 포함하는 비중입니다. 유한 phase register에서 해당 에너지를 목표 구간 안에 맞게 판독할 조건부 성공확률은 별도로 곱해집니다.

:::

실제 구현은 <span class="math-inline">Û=exp[−<var>i</var>(Ĥ−<var>E</var><sub>ref</sub>)<var>t</var>/ℏ]</span>처럼 energy shift와 evolution time을 정해 목표 spectrum이 하나의 `2π` phase 구간에 들어가도록 합니다. 이 범위를 잡지 않으면 서로 다른 에너지가 같은 phase로 접히는 aliasing이 생길 수 있습니다. 표준 QPE에서는 controlled-Û, controlled-Û², controlled-Û⁴처럼 더 긴 진화를 적용하고 inverse QFT로 누적 위상을 이진수로 읽습니다. QFT가 분자 Hamiltonian을 푸는 것이 아니라, 이미 기록된 phase를 판독합니다.

이상적인 QPE는 에너지 정밀도 `ε`에 대해 coherent evolution 또는 query가 `O(1/ε)`로 증가합니다. 독립 Pauli 표본평균의 `O(1/ε²)`와 다른 지점입니다. 이는 상태 준비와 Hamiltonian simulation 구현비용, 오류보정 비용을 제외한 정밀도 의존성 비교입니다. 또한 FCI coefficient vector를 모두 저장하고 대각화하지 않고도 determinant 중첩에 Hamiltonian evolution을 적용할 수 있습니다. [Aspuru-Guzik 등의 초기 분자 QPE 연구](https://doi.org/10.1126/science.1113479)는 이 경로를 물과 LiH에 대해 구성했습니다.

<details class="deep-dive" markdown="1">
<summary>심화 · QPE의 비용을 좌우하는 Hamiltonian simulation</summary>

실제 자원은 QFT보다 Hamiltonian simulation이 좌우합니다. Product formula(Trotterization), linear-combination-of-unitaries, qubitization 가운데 무엇을 쓰는지, 적분을 어떻게 factorize하는지에 따라 gate 수가 달라집니다. Block encoding은 정규화한 Hamiltonian `H/λ`를 더 큰 unitary 행렬의 한 block으로 구현하는 방법입니다. Qubitization은 각 고유값을 quantum walk의 회전 위상으로 바꾸며, QSP(Quantum Signal Processing)는 위상각의 연속을 조절해 시간진화나 spectral filter에 필요한 다항함수를 구현할 수 있습니다.

QPE는 선택한 Hamiltonian simulation 또는 quantum walk가 만든 고유위상을 에너지로 읽습니다. 대표적인 qubitization 기반 경로는 `integral → block encoding → qubitization → QPE`이며, 필요한 변환에 QSP를 결합할 수 있습니다. QSP와 qubitization은 VQE·QPE와 경쟁하는 별도 전자구조 solver가 아닙니다. [Low와 Chuang의 qubitization 연구](https://doi.org/10.22331/q-2019-07-12-163)는 이 oracle model의 대표 경로를 제시합니다. 이 경로의 실제 자원에는 normalization `λ`, PREPARE·SELECT, 초기상태 overlap과 오류보정 비용이 들어갑니다.
...
<figure class="figure-panel">
  <img class="diagram-desktop" src="vqe_qpe_mechanism_comparison.svg" alt="VQE의 짧은 매개변수 회로와 반복 Pauli 측정 및 고전 최적화 루프를 QPE의 초기상태 준비, 긴 controlled Hamiltonian evolution, inverse QFT 위상 판독과 비교한 도식">
  <img class="diagram-mobile" src="vqe_qpe_mechanism_comparison_mobile.svg" alt="모바일 화면용 세로 배열로 재구성한 VQE와 QPE의 작동 원리 및 비용 비교 도식">
  <figcaption><strong>그림 4.</strong> VQE는 짧은 회로와 많은 shots·optimizer iteration을 교환하고, QPE는 좋은 초기상태와 오류보정된 긴 coherent evolution을 요구합니다. 둘의 잠재 이득과 실패 원인이 다르므로 qubit 수 하나로 비교할 수 없습니다.</figcaption>
</figure>
...
정밀 검사 장비의 내부 상태를 모두 복사할 필요는 없습니다. 다음 공정이 판정에 쓸 검사 결과만 꺼내야 합니다. QPU의 상태 amplitude를 모두 내려받는 tomography는 지수적으로 비싸므로, 재료 계산은 다음 의사결정에 필요한 관측량을 먼저 정합니다. 1-RDM <span class="math-inline"><var>γ</var><sub>pq</sub>=⟨â<sup>†</sup><sub>p</sub>â<sub>q</sub>⟩</span>와 2-RDM <span class="math-inline"><var>Γ</var><sub>pqrs</sub>=⟨â<sup>†</sup><sub>p</sub>â<sup>†</sup><sub>q</sub>â<sub>s</sub>â<sub>r</sub>⟩</span>으로 one·two-body energy, 전자·spin density, dipole, orbital gradient와 embedding quantity를 구성할 수 있습니다. 다만 전체 2-RDM 원소 수도 `O(K⁴)`이므로 모든 것을 읽으면 측정 이득이 사라질 수 있습니다.
...
[2024년 MgO 결함 연구](https://www.nature.com/articles/s41524-024-01477-2)는 periodic range-separated DFT 환경 안의 2전자·5-orbital fragment를 VQE와 qEOM으로 풀었습니다. 원자 배치와 장거리 환경은 고전 계산이, 국소 결함 상태는 작은 양자 회로가 맡았습니다. 고전 FCI로 전 범위를 확인할 수 있는 크기였으며, 이 결과가 입증한 범위는 **DFT–QPU 인터페이스의 동작**입니다. 문제 크기에 따른 우위는 시험하지 않았습니다.
...
직관적으로 quantum kernel은 후보를 **새 좌표계에 펼쳐 유사도를 다시 재는 방법**입니다. 기존 표현에서는 멀리 떨어져 보이던 두 후보가 양자 feature map에서는 비슷한 패턴을 보일 수 있습니다. 그러나 좌표계가 복잡하다는 사실만으로 좋은 지도가 되지는 않습니다. 미학습 계열의 예측과 불확실성 보정이 실제로 좋아져야 의미가 있습니다.
...
표본추출기는 정답 하나를 찍는 장치보다 **확률이 설계된 주사위**에 가깝습니다. 좋은 주사위는 인기 있는 후보만 되풀이하지 않고, 목표 분포의 여러 mode를 적절한 비율로 보여주며, 유효하고 다양한 후보를 냅니다. 최저 비용 후보 하나를 찾는 최적화와 평가 기준이 다른 이유입니다.

QCBM은 회로가 만든 Born probability를 학습하고, QBM은 Hamiltonian의 Gibbs 분포를 모델링합니다. 고전 RBM의 표본추출을 양자 어닐러에 맡기는 방식은 또 다른 범주입니다. 세 방법을 모두 “양자 생성모델”로 뭉치면 무엇이 개선됐는지 알기 어렵습니다. 특히 실제 annealer가 목표 Gibbs 분포를 그대로 낸다고 가정해서는 안 됩니다.
...
VQE·QPE와 QA·QAOA를 같은 “에너지 최소화”로 묶으면 계산 대상이 흐려집니다. 전자구조 Hamiltonian <span class="math-inline">Ĥ<sub>e</sub></span>의 바닥상태는 correlated electronic state이고, 그 고유값이 전자 에너지입니다. 반면 QUBO에서 만든 cost Hamiltonian <span class="math-inline">Ĥ<sub>C</sub></span>의 바닥상태는 대리모델 점수와 제약 penalty가 가장 낮은 **설계 bitstring**입니다. 이 energy는 실제 물질의 전자 에너지가 아니라 후보의 목적함수 값입니다.

DFT·TDDFT·VQE·QPE가 만든 에너지, gap, 전하분포와 transition property는 이 두 번째 단계에서 자료나 label로 쓰입니다. 연구자는 치환기 위치, donor·acceptor·dopant 선택, 조성 구간, 공정 조건 또는 다음 실험 batch를 <span class="math-inline"><var>x</var><sub>i</sub>∈{0,1}</span>로 정의합니다. 이 시점부터 문제는 파동함수 계산이 아니라 제한된 예산에서 좋은 조합을 고르는 이산 의사결정입니다.

::: thinking 같은 Hamiltonian이라는 이름, 다른 물리

- 전자구조 <span class="math-inline">Ĥ<sub>e</sub></span>: `I`, `X`, `Y`, `Z`가 여러 qubit에 걸쳐 곱으로 이어진 다양한 Pauli string의 합이며, 일반적으로 서로 commute하지 않습니다. VQE·QPE가 eigenstate와 eigenvalue를 구합니다.
- 설계 비용 <span class="math-inline">Ĥ<sub>C</sub></span>: 기본적으로 <span class="math-inline">Î, Ẑ<sub>i</sub>, Ẑ<sub>i</sub>Ẑ<sub>j</sub></span> 항만 있는 대각 Hamiltonian입니다. QA·QAOA와 고전 optimizer가 낮은 비용 bitstring을 찾습니다.

Alq₃ 연구의 “VQE”처럼 generic eigensolver를 대각 Ising 비용함수에 적용할 수도 있습니다. 이것은 전자구조 VQE와 역할이 다릅니다.

:::

### QUBO는 물성 대리모델과 제약을 하나의 비용함수로 묶습니다

QUBO는 후보를 평가하는 **점수표**로 생각하면 쉽습니다. 대각 성분은 치환기나 공정 조건 하나를 골랐을 때의 개별 점수이고, 비대각 성분은 둘을 함께 골랐을 때 생기는 상승효과나 충돌을 적습니다. 제약 위반에는 벌점을 더합니다. 이 점수표가 원래 물성과 제약을 충실히 옮겼는지가 solver보다 먼저 검증돼야 합니다.
...
::: evidence 설계 문제의 QUBO 정식화

<div class="math-display" role="math" aria-label="QUBO 목적함수는 이차 점수와 제약 위반 벌점의 합입니다">
  <div class="math-row"><var>C</var>(<var>x</var>) = <var>x</var><sup>T</sup><var>Qx</var> + <span class="sum">∑</span><sub>a</sub> <var>λ</var><sub>a</sub> <var>g</var><sub>a</sub>(<var>x</var>)<sup>2</sup><span class="condition"><var>x</var><sub>i</sub> ∈ {0, 1}</span></div>
</div>

여기서 <span class="math-inline"><var>g</var><sub>a</sub>(<var>x</var>)</span>는 exactly-one이나 선형 budget equality 같은 **선형 등식의 residual**입니다. Mutual exclusion은 보통 <span class="math-inline"><var>λx</var><sub>i</sub><var>x</var><sub>j</sub></span>로, 부등식은 slack bit를 도입해 처리합니다. <span class="math-inline"><var>λ</var><sub>a</sub></span>가 작으면 무효해가 남고, 너무 크면 유효한 물성 차이가 제한된 coefficient precision에서 눌립니다.

:::

Factorization machine처럼 단일 선택 효과와 pair interaction을 이미 2차식으로 예측하는 대리모델은 QUBO와 직접 연결됩니다. OLED라면 donor와 acceptor를 각각 하나씩 고르는 제약, 함께 쓰지 못하는 조합, 합성비용과 예측 효율을 하나의 식에 넣을 수 있습니다. 하지만 GNN·DFT 결과를 2차 surrogate로 근사했다면 solver 오차보다 **정식화 오차**가 먼저 커질 수 있습니다.
...
<span class="math-inline"><var>x</var><sub>i</sub>=(1−<var>z</var><sub>i</sub>)/2</span>, <span class="math-inline"><var>z</var><sub>i</sub>∈{−1,+1}</span>로 치환하면 QUBO는 Ising 에너지로 바뀝니다. 이를 qubit operator로 쓰면 모든 항이 computational basis에서 대각이고 서로 commute합니다.

::: evidence QUBO에서 Ising cost Hamiltonian으로

<div class="math-display" role="math" aria-label="이진 변수를 이징 스핀으로 바꾸고 얻은 비용 해밀토니안">
  <div class="math-row"><var>x</var><sub>i</sub> = <span class="frac"><span>1 − <var>z</var><sub>i</sub></span><span>2</span></span><span class="condition"><var>z</var><sub>i</sub> ∈ {−1, +1}</span></div>
  <div class="math-row">x̂<sub>i</sub> = <span class="frac"><span>Î − Ẑ<sub>i</sub></span><span>2</span></span></div>
  <div class="math-row">Ĥ<sub>C</sub> = <var>C</var>(x̂) = <var>c</var><sub>0</sub> Î + <span class="sum">∑</span><sub>i</sub> <var>h</var><sub>i</sub> Ẑ<sub>i</sub></div>
  <div class="math-row math-indent">+ <span class="sum">∑</span><sub>i&lt;j</sub> <var>J</var><sub>ij</sub> Ẑ<sub>i</sub>Ẑ<sub>j</sub></div>
</div>

각 basis bitstring이 설계 후보 하나이고, eigenvalue가 그 후보의 QUBO 점수입니다. QA와 QAOA의 차이는 같은 <span class="math-inline">Ĥ<sub>C</sub></span>의 낮은 에너지 bitstring을 어떤 동역학으로 찾느냐에 있습니다.

:::

<figure class="figure-panel">
  <img class="diagram-desktop" src="qubo_qa_qaoa_bridge.svg" alt="재료 설계변수와 제약을 QUBO로 만들고 Ising cost Hamiltonian으로 변환한 뒤 양자 어닐링과 QAOA가 각각 낮은 비용 bitstring을 찾고 고정밀 검증으로 보내는 계산 흐름">
  <img class="diagram-mobile" src="qubo_qa_qaoa_bridge_mobile.svg" alt="모바일 화면용 세로 배열로 재구성한 QUBO에서 QA와 QAOA 및 재료 검증으로 이어지는 흐름">
  <figcaption><strong>그림 5.</strong> QUBO는 물성 자료와 설계 제약을 이진 비용함수로 컴파일합니다. QA와 QAOA는 같은 비용 Hamiltonian을 서로 다른 방식으로 탐색하며, 나온 bitstring은 반드시 원래의 고정밀 계산과 실험 조건에서 다시 검증해야 합니다.</figcaption>
</figure>

### QA는 transverse field를 줄이며 낮은 에너지 해를 표본화합니다

Quantum annealing은 QA에 한정해 울퉁불퉁한 비용 지형을 낮은 곳으로 이동하는 그림으로 이해할 수 있습니다. Transverse field가 큰 시작점에서는 여러 bitstring이 함께 섞여 있고, 이를 줄이면서 문제 Hamiltonian의 비중을 높입니다. 이상적인 닫힌 계에서는 바닥상태 경로를 천천히 따라가지만, 실제 장치는 열과 잡음이 있는 open system이므로 결과는 정답 한 개가 아니라 낮은 에너지 표본의 분포입니다.

::: evidence 양자 어닐링의 표준 경로

<div class="math-display" role="math" aria-label="양자 어닐링 해밀토니안은 드라이버와 비용 해밀토니안의 시간 의존 가중합입니다">
  <div class="math-row"><var>s</var> = <var>t</var>/<var>t</var><sub>f</sub> ∈ [0, 1]</div>
  <div class="math-row">Ĥ(<var>s</var>) = <var>A</var>(<var>s</var>) Ĥ<sub>D</sub> + <var>B</var>(<var>s</var>) Ĥ<sub>C</sub><span class="condition">Ĥ<sub>D</sub> = −<span class="sum">∑</span><sub>i</sub> X̂<sub>i</sub></span></div>
  <div class="math-row"><var>A</var>(0) ≫ <var>B</var>(0)<span class="condition"><var>A</var>(1) ≪ <var>B</var>(1)</span></div>
</div>

이상적 시작 한계 `B(0)=0`에서 바닥상태는 모든 bitstring에 퍼진 <span class="math-inline">|+⟩<sup>⊗n</sup></span>입니다. 닫힌 계에서 충분히 천천히 변화하면 바닥상태를 따라가지만, 필요한 시간은 경로의 최소 energy gap에 민감합니다.

:::

Transverse field가 만드는 quantum fluctuation과 tunneling은 높고 폭이 좁은 장벽에 갇힌 상태가 다른 basin으로 이동하도록 도울 수 있습니다. [Kadowaki와 Nishimori의 원 연구](https://doi.org/10.1103/PhysRevE.58.5355)는 transverse Ising model로 이 생각을 제시했고, [2016년 multiqubit tunneling 실험](https://www.nature.com/articles/ncomms10327)은 의도적으로 만든 문제에서 여러 qubit의 collective tunneling이 계산 과정에 참여함을 보였습니다. 그 논문 자체도 quantum speedup을 확립한 것은 아니라고 선을 그었습니다. 터널링은 메커니즘의 증거이지 종단 이득의 증거가 아닙니다.
...
QAOA는 공이 비용 지형 위를 지나가는 경로로 그리면 오해하기 쉽습니다. 먼저 cost 연산이 각 bitstring에 비용에 따른 **위상표식**을 붙입니다. 이때 측정확률은 아직 바뀌지 않습니다. 이어 mixer가 후보들의 amplitude를 다시 섞으면 위상 차이가 간섭으로 바뀌고, 일부 좋은 후보의 최종 표본확률이 커질 수 있습니다. 이 `cost → mixer` 교대를 `p`회 적용하고, 고전 optimizer가 `2p`개의 각도를 조정합니다.

::: evidence 깊이 p의 QAOA 상태

<div class="math-display" role="math" aria-label="깊이 p의 QAOA 상태와 비용 및 믹서 유니터리">
  <div class="math-row">|<var>γ</var>,<var>β</var>⟩ = Û<sub>M</sub>(<var>β</var><sub>p</sub>) Û<sub>C</sub>(<var>γ</var><sub>p</sub>) ⋯</div>
  <div class="math-row math-indent">Û<sub>M</sub>(<var>β</var><sub>1</sub>) Û<sub>C</sub>(<var>γ</var><sub>1</sub>) |+⟩<sup>⊗n</sup></div>
  <div class="math-row">Û<sub>C</sub>(<var>γ</var>) = exp(−<var>iγ</var>Ĥ<sub>C</sub>)</div>
  <div class="math-row">Û<sub>M</sub>(<var>β</var>) = exp(−<var>iβ</var>Ĥ<sub>M</sub>)<span class="condition">Ĥ<sub>M</sub> = <span class="sum">∑</span><sub>i</sub> X̂<sub>i</sub></span></div>
</div>

Cost 연산만 적용하면 각 bitstring에는 에너지별 phase만 붙고 측정확률은 그대로입니다. Mixer가 bitstring 사이를 섞을 때 phase 차이가 interference로 바뀌면서 좋은 해의 amplitude가 커질 수 있습니다.

:::

[Farhi·Goldstone·Gutmann의 원 논문](https://arxiv.org/abs/1411.4028)에서 approximation quality는 depth `p`에 따라 개선될 수 있지만 회로 깊이도 함께 증가합니다. 각 QUBO edge의 `ZZ` evolution에는 entangling gate가 필요하고, hardware에 없는 연결에는 SWAP routing이 더해집니다. `p`가 커지면 표현력과 함께 two-qubit error, shots, parameter optimization과 compilation 비용도 증가합니다. 낮은 `p`에서는 회로의 국소성 때문에 멀리 떨어진 제약 구조를 충분히 반영하지 못할 수 있습니다. 실제로 [Bravyi 등의 연구](https://doi.org/10.1103/PhysRevLett.125.260505)는 특정 MaxCut 계열에서 모든 constant-depth 표준 QAOA보다 강한 고전 알고리즘이 나은 한계를 보였습니다.
...
<figure class="figure-panel">
  <img class="diagram-desktop" src="qa_qaoa_search_intuition.svg" alt="같은 QUBO 비용과 제약에서 양자 어닐링은 transverse-field schedule과 open-system 동역학으로, QAOA는 cost phase와 mixer 간섭으로 낮은 비용 bitstring 표본을 만들고 둘 다 고전 비교군과 종단 검증을 거치는 비교 도식">
  <img class="diagram-mobile" src="qa_qaoa_search_intuition_mobile.svg" alt="모바일 화면용 세로 배열로 재구성한 양자 어닐링과 QAOA의 서로 다른 표본 생성 원리 및 공통 검증 기준">
  <figcaption><strong>그림 6.</strong> QA와 QAOA는 같은 QUBO 점수표를 받아도 같은 방식으로 움직이지 않습니다. QA의 비용 지형과 tunneling은 제한된 직관이며, QAOA는 후보의 이동 경로가 아니라 cost phase와 mixer가 바꾸는 최종 표본분포로 이해해야 합니다. 두 방법 모두 강한 고전 비교군과 고정밀·실험 재검증을 통과해야 합니다.</figcaption>
</figure>
...
서로 독립인 각 실행이 성공확률 <span class="math-inline"><var>p</var><sub>s</sub></span>를 갖고 <span class="math-inline">0&lt;<var>p</var><sub>s</sub>&lt;1</span>이라고 가정하면, 99% 확률로 한 번 이상 목표 품질의 해를 얻는 데 필요한 실행 횟수는 다음과 같습니다.

<div class="math-display" role="math" aria-label="99 퍼센트 성공확률에 필요한 독립 실행 횟수">
  <div class="math-row"><var>N</var><sub>99</sub> = ⌈ <span class="frac"><span>ln(0.01)</span><span>ln(1 − <var>p</var><sub>s</sub>)</span></span> ⌉</div>
</div>

경계에서는 <span class="math-inline"><var>p</var><sub>s</sub>=0</span>이면 필요한 횟수가 무한대이고, <span class="math-inline"><var>p</var><sub>s</sub>=1</span>이면 한 번입니다. 실행들이 상관돼 있거나 장치 분포가 시간에 따라 변하면 이 식을 그대로 적용할 수 없습니다.

그러나 실행시간만 빠른 QUBO 최저점이 실제 물성에서 실패하면 의미가 없습니다. 재료 파이프라인에서는 `고정밀 검증 1회당 통과한 고유 후보 수`와 전체 wall-clock·비용을 함께 보는 편이 더 직접적입니다. [Rønnow 등의 speedup 판정 연구](https://doi.org/10.1126/science.1252319)는 comparator 선택과 scaling을 잘못 잡으면 speedup 결론이 바뀔 수 있음을 정리했습니다.

### 정식화 오차, solver 오차와 재료 오차를 분리합니다

여기서는 비용을 **최소화**한다고 두겠습니다. 고정밀 평가함수를 <span class="math-inline"><var>F</var>(<var>x</var>)</span>, QUBO 대리함수를 <span class="math-inline"><var>E</var><sub>Q</sub>(<var>x</var>)</span>라고 하겠습니다. <span class="math-inline"><var>x</var><sup>★</sup></span>는 실제 평가의 최적해, <span class="math-inline"><var>x</var><sub>Q</sub><sup>★</sup></span>는 QUBO 최적해, <span class="math-inline"><var>x</var><sub>sol</sub></span>은 solver가 찾은 해입니다.

<div class="math-display" role="math" aria-label="QUBO 정식화 오차와 솔버 오차 및 종단 오차의 정의">
  <div class="math-row"><var>x</var><sup>★</sup> = arg min<sub>x∈X<sub>feas</sub></sub> <var>F</var>(<var>x</var>)</div>
  <div class="math-row"><var>x</var><sub>Q</sub><sup>★</sup> = arg min<sub>x∈{0,1}<sup>n</sup></sub> <var>E</var><sub>Q</sub>(<var>x</var>)</div>
  <div class="math-row"><var>R</var><sub>form</sub> = <var>F</var>(<var>x</var><sub>Q</sub><sup>★</sup>) − <var>F</var>(<var>x</var><sup>★</sup>)</div>
  <div class="math-row"><var>Δ</var><sub>solver</sub> = <var>E</var><sub>Q</sub>(<var>x</var><sub>sol</sub>) − <var>E</var><sub>Q</sub>(<var>x</var><sub>Q</sub><sup>★</sup>)</div>
  <div class="math-row"><var>R</var><sub>end</sub> = <var>F</var>(<var>x</var><sub>sol</sub>) − <var>F</var>(<var>x</var><sup>★</sup>)</div>
</div>

| 오차 | 묻는 질문 |
| --- | --- |
| 정식화 regret, <span class="math-inline"><var>R</var><sub>form</sub></span> | QUBO 최적해가 원래 평가함수에서도 좋은가 |
| solver gap, <span class="math-inline"><var>Δ</var><sub>solver</sub></span> | 주어진 QUBO를 얼마나 잘 풀었는가 |
| 종단 regret, <span class="math-inline"><var>R</var><sub>end</sub></span> | 최종 후보가 실제 평가에서도 좋은가 |

<span class="math-inline"><var>x</var><sup>★</sup></span>는 정의상 원래 문제의 feasible domain 안에 있습니다. 반면 QUBO가 고른 <span class="math-inline"><var>x</var><sub>Q</sub><sup>★</sup></span>와 solver 출력 <span class="math-inline"><var>x</var><sub>sol</sub></span>은 제약을 위반할 수 있으며, 이때 원래 평가에서는 <span class="math-inline"><var>F</var>=∞</span> 또는 사전에 정한 infeasibility penalty를 적용합니다. 정식화 regret가 크면 QA나 QAOA가 QUBO를 정확히 풀어도 좋은 물질을 얻지 못합니다. 다만 <span class="math-inline"><var>R</var><sub>form</sub></span>은 최적해 하나의 이전 품질을 볼 뿐 전체 후보 순위를 측정하지 않습니다. 원래 평가함수와 QUBO가 후보 순서를 얼마나 보존하는지는 Kendall `τ`, Spearman `ρ`, top-k overlap도 함께 봐야 합니다. 계산화학자와 재료전문가가 양자 최적화 연구에서 맡을 가장 중요한 역할은 이 경계를 검증하는 일입니다.

분자 설계의 선행 사례에도 이 구분이 필요합니다. [Ajagekar와 You의 2023년 연구](https://www.nature.com/articles/s41524-023-01099-0)는 GraphConv fingerprint, conditional RBM, QUBO와 D-Wave를 연결했습니다. [2026년 Digital Discovery 연구](https://doi.org/10.1039/D6DD00012F)는 고정 CDDD 표현과 선형 예측기를 QUBO에 연결하고 hybrid BQM 해법과 SA를 비교했습니다. 일부 다목적 조건에서 hybrid 해법의 공동 목표 충족률이 높았지만, 문제 분해와 고전 heuristic을 포함하므로 QPU 단독의 우위로 해석할 수 없습니다. 해독 분자의 validity도 51.9–54.1%였고 latent objective와 실제 물성의 차이가 남았습니다.

OLED와 맞닿은 [2023년 Alq₃ 중수소 치환 연구](https://spj.science.org/doi/10.34133/icomputing.0037)는 6개 위치를 6 bit, 즉 64개 조합으로 표현하고 고전 양자화학·factorization machine·Ising cost Hamiltonian·VQE/QAOA를 연결했습니다. 여기서 VQE는 전자구조 Hamiltonian이 아니라 6-qubit 대각 비용 Hamiltonian의 바닥상태를 찾았습니다. 실제 장치의 초기 성공확률은 VQE 0.83, QAOA 0.075였고, QAOA는 측정확률을 이용한 재귀적 변수 제거와 고전 후처리 뒤 약 0.97까지 올라갔습니다. 따라서 이 연구는 계산 단계의 연결 방법과 hybrid reduction의 효과를 보여주는 좋은 PoC이지만 scaling advantage의 증거는 아닙니다.
...
1. **표현 용량**은 `n` qubit이 `2ⁿ`차원 Hilbert space의 상태를 표현하거나, 여러 bitstring에 amplitude를 둘 수 있다는 뜻입니다. 고정 전자수 물리 sector는 <span class="math-inline">C(<var>n</var>,<var>N</var><sub>e</sub>)</span>이며, state preparation과 readout이 비효율적이면 여기서 멈춥니다.
2. **알고리즘 이득**은 명시된 입력 oracle과 정확도 가정에서 gate·query·time complexity가 강한 고전 방법보다 유리하다는 뜻입니다. QPE의 phase accumulation이 대표적인 조건부 사례입니다.
3. **종단 이득**은 basis·active space, encoding, compilation, QPU 실행, shots, 오류제어, queue, decoding과 고정밀 검증을 모두 포함해 시간·비용 또는 검증 성공률이 개선됐다는 뜻입니다.

<figure class="figure-panel">
  <img class="diagram-desktop" src="quantum_advantage_ledger.svg" alt="큰 양자 상태공간의 표현 이득에서 알고리즘 복잡도 이득을 거쳐 실제 재료 검증의 종단 이득으로 나아가며 상태 준비, 측정, 오류보정, 임베딩과 검증에서 이득이 사라질 수 있음을 보여주는 도식">
  <img class="diagram-mobile" src="quantum_advantage_ledger_mobile.svg" alt="모바일 화면용 세로 배열로 재구성한 양자 표현, 알고리즘, 종단 이득의 세 단계와 비용 누수 지점">
  <figcaption><strong>그림 7.</strong> 표현 용량은 출발점이고 알고리즘 이득은 조건부 약속입니다. 전처리와 관측량 readout, 오류제어 및 재료 검증을 통과해 동일 예산에서 가치가 남아야 종단 양자 이득입니다.</figcaption>
</figure>
...
전체 소요시간은 다음 범위를 빠짐없이 기록합니다. 여기서 각 항은 서로 겹치지 않게 집계하고, optimizer가 QPU를 여러 번 호출한다면 각 job의 compilation·queue·실행·측정을 모두 더합니다.

<div class="math-display" role="math" aria-label="전처리부터 반복 양자 작업과 사후처리 및 검증까지 포함한 전체 소요시간">
  <div class="math-row"><var>T</var><sub>total</sub> = <var>T</var><sub>pre</sub> + <var>T</var><sub>encode</sub> + <var>T</var><sub>post</sub> + <var>T</var><sub>validate</sub></div>
  <div class="math-row math-indent">+ <span class="sum">∑</span><sub>r=1</sub><sup>N<sub>jobs</sub></sup> [<var>T</var><sub>compile</sub><sup>(r)</sup> + <var>T</var><sub>queue</sub><sup>(r)</sup></div>
  <div class="math-row math-indent">+ <var>T</var><sub>QPU</sub><sup>(r)</sup> + <var>T</var><sub>measure</sub><sup>(r)</sup></div>
  <div class="math-row math-indent">+ <var>T</var><sub>optimizer</sub><sup>(r)</sup>]</div>
</div>

<span class="math-inline"><var>T</var><sub>optimizer</sub><sup>(r)</sup></span>에는 목적함수 집계와 다음 회로 매개변수 업데이트처럼 job 사이에서 수행되는 고전 최적화 시간을 넣습니다. 여러 job이 병렬로 실행된다면 단순 합이 아니라 의사결정이 끝날 때까지의 critical-path wall time을 별도로 보고합니다. 금액, 에너지 사용량과 인력비도 시간과 섞지 않고 독립된 비용 장부에 기록해야 합니다.
...
후보를 고를 때 평균 물성만큼 실패확률도 중요합니다. 고전 Monte Carlo가 많은 표본을 뽑아 위험과 기대효용의 평균을 추정한다면, Quantum Amplitude Estimation(QAE)은 그 확률분포와 utility를 coherent amplitude로 준비할 수 있을 때 더 적은 oracle 호출로 평균을 추정하려는 방법입니다. 이론적으로 query 수를 거의 제곱근 수준으로 줄일 가능성이 있으며, [Montanaro의 2015년 연구](https://pmc.ncbi.nlm.nih.gov/articles/PMC4614442/)는 이 일반적인 quantum Monte Carlo 경로를 제시했습니다.
...
<figure class="figure-panel">
  ...
  <figcaption><strong>그림 8.</strong> E0의 이론적 speedup과 E4의 반복 가능한 설계 가치는 다른 주장입니다. 이 리뷰에서 확인한 공개 재료·분자 사례는 대체로 E1 소형 실증 또는 제한된 E2 도메인 벤치마크에 있으며, 고정밀 계산이나 실험을 사전에 정한 기준으로 통과하는 E3·E4가 다음 과제입니다.</figcaption>
</figure>

<details class="deep-dive" markdown="1">
<summary>심화 · 모듈별 현재 증거와 다음 검증 질문</summary>
...
이 리뷰에서 확인한 공개 근거만 보면, 어느 모듈도 재료 역설계 전반의 양자우위를 입증했다고 보기 어렵습니다. 모듈별 증거 수준은 서로 다르므로, 작은 입력·출력·오차 조건을 고정하고 증거 사다리의 다음 한 칸을 목표로 삼아야 합니다.

여기서 `E0–E4`는 공개 연구가 어디까지 입증했는지를 나타내는 **외부 증거 수준**입니다. 뒤의 `B0–B5`는 한 연구팀이 검증을 어떤 순서로 수행할지를 나타내는 **내부 벤치마크 순서**입니다.
...
<figure class="figure-panel">
  ...
  <figcaption><strong>그림 9.</strong> 이 유즈케이스는 donor·acceptor co-host가 만든 charge-transfer exciplex, 별도의 phosphorescent dopant, 박막과 소자 조건을 함께 다룹니다. 그림은 이 관계를 일반화한 편집 일러스트레이션입니다.</figcaption>
</figure>
...
이 사례에서 후보 작업지시서에는 단일 분자만 적히지 않습니다. Donor·acceptor·dopant의 조합, 분자쌍의 거리와 배향, 박막 조건, 소자 recipe와 예상 불확실성이 함께 들어갑니다. 전자구조·대리모델·표본추출·조합선택 모듈은 이 작업지시서의 서로 다른 항목을 다루고, 최종 출하 판정은 박막과 소자 실험이 맡습니다.
...
<figure class="figure-panel">
  ...
  <figcaption><strong>그림 10.</strong> 같은 donor–acceptor 조합도 거리, 배향과 주변 packing에 따라 전하이동 상태와 dopant로의 에너지 전달이 달라집니다. 따라서 개별 분자, pair geometry, 계면과 소자 조건을 하나의 일관된 자료 묶음으로 관리해야 합니다.</figcaption>
</figure>
...
네 양자 모듈을 동시에 넣으면 결과가 좋아져도 원인을 알 수 없습니다. 첫 PoC는 한 모듈의 입력·출력·허용오차를 고정하고 나머지는 같은 고전 파이프라인으로 유지해야 합니다.
...
1. DFT·TDDFT가 어려워하는 다중참조 상태, bond breaking, transition-metal center, defect state 또는 CT state ordering을 한 가지 고릅니다.
2. Geometry, basis·pseudopotential, embedding과 active space를 고정하고 one·two-electron integral을 보존합니다.
...
1. 연속변수와 이산변수를 분리하고, 선택·배치·치환·batch scheduling처럼 실제 결정을 나타내는 변수만 QUBO 후보로 둡니다.
2. 대리모델과 제약을 QUBO로 옮긴 뒤 작은 문제에서 원래 평가함수와 optimum·candidate ranking을 비교합니다.
3. Exact enumeration, CP-SAT·MILP, branch-and-bound, SA·Tabu·parallel tempering을 같은 QUBO와 같은 tuning budget에 연결합니다.
4. QA에는 minor embedding, chain break, gauge, programming·readout·decoding을 포함합니다.
5. QAOA에는 depth `p`, mixer, routing, shots와 고전 optimizer의 모든 호출을 포함합니다.
6. 최종 순위는 QUBO objective가 아니라 고정밀 재계산 또는 실험을 통과한 후보 수로 다시 정합니다.
...
- **생성·표본추출 트랙**은 두 수준을 분리합니다. 종단 생성에서는 같은 자료와 고정밀 계산 또는 실험 검증 기준 아래에서 QCBM·QBM hybrid generator를 VAE·diffusion·autoregressive model과 비교합니다. 표본추출만 시험할 때는 같은 RBM 또는 energy model을 고정하고 annealer를 Gibbs sampling·MCMC·parallel tempering과 비교합니다.
- **최적화 트랙**은 같은 QUBO 또는 명시된 제약 조건을 CP-SAT·MILP·SA·Tabu, QA와 QAOA에 연결합니다. 가능한 작은 문제는 exact enumeration으로 optimum과 gap을 확인합니다.
...
<figure class="figure-panel">
  ...
  <figcaption><strong>그림 11.</strong> 후보의 가치는 고정밀 계산과 실제 검증을 통과하면서 드러납니다. OLED에서는 박막·소자 시험이 마지막 기준이지만, 다른 재료 분야에서는 합성, 촉매 활성, 전지 수명 또는 기계 특성 시험이 이 기준을 맡습니다. 탈락 이유도 다음 학습 자료로 돌려보냅니다.</figcaption>
</figure>
...
반대로 여러 seed와 문제 크기에서 개선이 반복되고, 같은 검증 예산으로 더 많은 유효 후보를 남기며, 전체 비용에서도 이득이 유지된다면 증거 사다리의 다음 단계로 이동할 수 있습니다. 결론의 범위는 “이렇게 고정한 입력·출력·오차 조건에서 이 양자 서브루틴이 유효했다”로 제한합니다. 이를 양자 파이프라인 전체의 승리로 넓히지는 않습니다.

::: thinking 이 전환이 만들 수 있는 가장 큰 변화

장기적 가치의 핵심은 DFT 계산 한 건을 단순히 더 빨리 끝내는 데 있지 않습니다. 지금은 비용 때문에 설계 후반의 소수 후보에만 적용하는 상관 전자·여기상태 판정을 더 앞 단계와 더 넓은 후보군에 되돌려 놓는 데 있습니다. 여기에 분포를 보존하는 표본추출과 제약을 반영한 선택이 연결되면, 파이프라인은 “많이 만들고 뒤늦게 탈락시키는” 방식에서 “불확실성을 보면서 다음 계산과 실험을 고르는” 방식으로 바뀔 수 있습니다.

그때의 가장 큰 연구적 이득은 지금 계산비용 때문에 일찍 닫아 둔 설계공간을 다시 여는 것입니다. 다만 그 미래를 증명하는 단위는 큐비트 수나 한 번의 QPU 실행시간이 아닙니다. **같은 전체 예산에서 더 다양하고, 실제 검증을 통과한 후보를 반복해서 남기는가**가 마지막 기준입니다.
...
30. [Seeley et al., The Bravyi–Kitaev transformation for quantum computation of electronic structure](https://doi.org/10.1063/1.4768229)
31. [Huggins et al., Efficient and noise resilient measurements for quantum chemistry](https://www.nature.com/articles/s41534-020-00341-7)
32. [Aspuru-Guzik et al., Simulated Quantum Computation of Molecular Energies](https://doi.org/10.1126/science.1113479)
33. [Kadowaki and Nishimori, Quantum Annealing in the Transverse Ising Model](https://doi.org/10.1103/PhysRevE.58.5355)
34. [Boixo et al., Computational multiqubit tunnelling in programmable quantum annealers](https://www.nature.com/articles/ncomms10327)
...
- 시각 자료: 사례 장면을 설명하는 원본 편집 일러스트레이션 3종과 desktop/mobile 결정론적 SVG 도식 8종
