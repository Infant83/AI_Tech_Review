...
## 2026-05-21 Distribution Package

- Checked the earlier `2026-05-09_ai-updates-weekly/dist_email` package and reused the same sharing pattern:
  - flat HTML distribution with `index.html`;
  - local images and SVGs copied next to the HTML;
  - relative references rewritten so the package can be opened from the folder.
- Created:
  - `dist/index.html`
  - `dist/2026-05-07_tabpfn-oled-manufacturing-foundation-model_final_review.html`
  - local figure assets and linked review notes under `dist/`
  - `dist.zip` for email attachment.
- Verification:
  - `scripts/html_to_dist.py` returned `[local-ref-check] ok`.
  - Playwright opened `dist/index.html` and captured a preview successfully.
  - `dist.zip` size was approximately 4.63 MB.
- Email:
  - Attempted classic Outlook COM sending first, but the local system exposes new Outlook (`olk.exe`) and the COM server failed with `CO_E_SERVER_EXEC_FAILURE`.
  - Sent the package through the configured Gmail connector to `hyun-jung.kim@lgdisplay.com`.
  - Gmail returned sent message id `19e4744470872c48`.
