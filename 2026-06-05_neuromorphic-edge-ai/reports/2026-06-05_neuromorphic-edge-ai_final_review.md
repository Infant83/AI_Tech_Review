...
<figure class="article-hero-figure">
  <img src="../artifacts/final_review/figures/imagegen/neuromorphic_everyday_ai_hero-web.png" alt="현관 센서, 스마트 카메라, earbuds, 웨어러블 패치의 작은 이벤트가 테이블 위 local edge chip으로 모이고 하나의 신호만 상위 장치로 올라가는 일러스트">
...
::: highlight
집 안의 presence sensor, earbuds의 wake-word, 아기 울음 감지 모듈, 웨어러블 생체신호 패치, 현관 smart camera는 모두 같은 질문을 던집니다. AI가 똑똑해지는 것만으로 충분한가, 아니면 센서가 너무 오래 켜져 있고 너무 많은 데이터를 보내는 비용부터 줄여야 하는가. 뉴로모픽 컴퓨팅은 이 두 번째 질문에서 의미가 커집니다.
:::
...
<figure class="figure-panel">
  <img src="../artifacts/final_review/figures/imagegen/neuromorphic_sensor_time_section-web.png" alt="밤 시간의 생활 공간에서 문 센서, 스마트 스피커, earbuds, 웨어러블 패치가 작은 edge chip으로 sparse event를 보내고 하나의 신호가 dim tablet으로 올라가는 일러스트">
...
<figure class="figure-panel">
  <img src="../artifacts/final_review/figures/imagegen/neuromorphic_mos2_lab_section-web.png" alt="현미경 아래 박막 센서 칩에 색 있는 빛이 닿고 sparse spike-like signal이 작은 synapse array chip으로 연결되는 실험실 일러스트">
...
::: evidence
현재 확인할 수 있는 범위는 MoS2 광전자 neuron과 MoS2/HZO ferroelectric synapse의 통합, rate/TTFS coding, 91.7%/93.5% task 결과입니다. 상용 vision module 수준의 검증이나 대규모 array 실증까지 보여준 연구는 아닙니다.
:::
...
둘째는 항상 듣는 오디오입니다. wake-word, baby cry, abnormal sound, hearing assistance는 오디오 전체를 계속 저장하거나 클라우드로 보내기 어려운 영역입니다. [Baek and Lee의 2024년 SNN sound 리뷰](https://pmc.ncbi.nlm.nih.gov/articles/PMC11362401/)는 sound recognition에서 SNN이 low power와 low latency를 요구하는 embedded real-time application에 맞는 후보라고 정리합니다. [Joya Design의 Pulsar 기반 consumer audio module 발표](https://www.innatera.com/newsroom/joya-design-takes-neuromorphic-chip-from-design-to-device-with-first-innatera-powered-consumer-audio-product-at-awe-china/)는 evaluation board 단계를 넘어 실제 consumer audio product 쪽으로 뉴로모픽 칩이 들어가는 신호로 읽힙니다. audio SNN은 아직 개발 도구와 데이터셋, 정확도 검증이 더 필요하지만, 항상 듣는 AI의 전력 문제를 설명하기에는 매우 좋은 사례입니다.

셋째는 웨어러블과 on-body computing입니다. 웨어러블 AI는 한 번 멋지게 추론하는 것보다 피부와 손목 위에서 오래 버티는 일이 중요합니다. [Nature Electronics의 2026년 stretchable neuromorphic circuit 논문](https://www.nature.com/articles/s41928-026-01639-8)은 on-body edge computing을 위한 stretchable neuromorphic circuit을 제안했습니다. 아직 연구 단계이지만, 생체신호를 센서 가까이에서 전처리하고 분류하려는 흐름은 스마트워치, 패치형 센서, 재활·돌봄 기기와 직접 닿아 있습니다.

넷째는 smart camera와 in-sensor vision입니다. 현관 카메라, 공용공간 모니터링, 저조도 보안 센서, XR 기기의 안쪽 카메라는 모든 frame을 계속 보낼수록 전력과 프라이버시 부담이 커집니다. [SynSense Speck](https://www.synsense.ai/products/speck-2/)처럼 Dynamic Vision Sensor와 SNN processor를 single chip으로 묶는 제품 신호가 나오는 이유도 여기에 있습니다. Wang et al.의 MoS2/HZO 논문은 같은 방향의 더 앞단, 즉 빛을 받은 자리에서 spike와 synapse 계산을 시작할 수 있는지 묻는 재료·소자 연구입니다.
...
<figure class="figure-panel figure-panel-fit">
  <img src="../artifacts/final_review/figures/neuromorphic_maturity_timeline.svg" alt="2023 IBM NorthPole, 2024 Intel Hala Point, 2025 Nature 리뷰와 NeuroBench, 2026 in-sensor vision 및 edge neuromorphic 연구를 연결한 타임라인">
...
- [Baek and Lee, SNN and sound: a comprehensive review of spiking neural networks in sound, Biomedical Engineering Letters, 2024](https://pmc.ncbi.nlm.nih.gov/articles/PMC11362401/)
